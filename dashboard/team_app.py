...
def _fact_rankdrop(con, team_filter_p):
...
def _fact_hero(con, team_filter, date_between):
    df = con.execute(f"""
...
def render_insight_bot(con, team, team_filter, date_between):
    team_filter_p = f"AND p.Team = '{team}'"
    builders = [
        ...
        lambda: _fact_rankdrop(con, team_filter_p),
        lambda: _fact_hero(con, team_filter, date_between),
    ]
    n = len(builders)
...
with st.sidebar:
    ...
    team_filter = f"AND Team = '{selected_team}'"
    date_between = f"BETWEEN '{start_date.isoformat()}' AND '{end_date.isoformat()}'"

    render_insight_bot(con, selected_team, team_filter, date_between)
